...
export default function ProductDetail() {
  ...
  useEffect(() => {
    setLoading(true);
    api
      .get(`http://localhost:5000/get-product/${id}`)
      .then(res => {
  ...
  async function orderNow() {
    try {
      ...
      await api.post(
        ...
        { productId: product._id || product.id, quantity: qty },
  ...
  return (
    <div className="max-w-3xl mx-auto mt-10 p-6 border rounded shadow">
                ...
                {product.detailImages.map((img, i) => (
                  <div key={i} className="overflow-hidden rounded">
                      <img src={`http://localhost:5000${img}`}
                        className="h-32 w-full object-cover transition-transform duration-300 hover:scale-125" />
                  </div>
                  ))}
              </div>
          )}
      ...
      <input type="number" min="1" value={qty}
        onChange={(e) => setQty(e.target.value)} className="border p-2 mt-4 w-24" />

      <button onClick={orderNow} disabled={ordering} className="bg-green-600 text-white p-2 px-4 rounded mt-4 block" >
